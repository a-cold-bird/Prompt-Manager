# Docker Compose V2 远程服务器更新指南

## 📋 更新概述

适用于 **Docker Compose V2** (新版本，使用 `docker compose` 命令)
...
在远程服务器编辑 Dockerfile 和 docker-compose.yml，参照本指南下方的文件内容。
...
| 日志 | `docker-compose logs web` | `docker compose logs web` |
| 执行命令 | `docker-compose exec web bash` | `docker compose exec web bash` |
| 查看状态 | `docker-compose ps` | `docker compose ps` |

**主要区别**: V2 使用 `docker compose` (不带连字符)

---

## ✨ V2 的优势

- 🎯 集成到 Docker CLI，无需单独安装
- ⚡ 性能更好
- 🔄 自动更新与 Docker 同步
- 📦 更小的文件体积
- 🔒 更好的安全性
